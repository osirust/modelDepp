...
Для быстрой проверки решения организаторами мы подготовили облегченный скрипт `inference.py`.
В финальной версии мы используем **Native MBConv Architecture** (нативная реализация без внешних зависимостей типа `timm`), чтобы гарантировать 100% воспроизводимость результата на любой машине.

### 1. Установка зависимостей
Минимальный набор библиотек для запуска инференса:

    • pip install torch torchvision pandas numpy pillow opencv-python
    
## 2. Загрузка весов
Из-за ограничений GitHub на размер файлов, финальные веса модели (submission_weights.pth) доступны по внешней ссылке.

👉 [СКАЧАТЬ ВЕСА МОДЕЛИ] (https://drive.google.com/file/d/18BqFtgiRZqwFjFwsvPmUYoY2H-jggAJk/view?usp=sharing) 
(Google Drive)
Важно: Скачайте файл и поместите его в корень репозитория рядом с inference.py.

## 3. Запуск

    • python inference.py

Скрипт автоматически найдет папку data/faces_384/test, обработает изображения и создаст файл submission.csv.

### 2. Эссе: Эволюция архитектур для детекции дипфейков
...
## 3. Финал: TitanNet V42 (Native) Опираясь на успех механизмов внимания, мы перешли к архитектуре TitanNet Native — это глубокий стек оптимизированных блоков Inverted Residual Blocks (MBConv) с кастомной проекционной головой. Ключевые улучшения:
 
    • Native Implementation: Полный отказ от внешних библиотек в пользу нативного PyTorch для стабильности.

    • SSL Pretraining (SimMIM): Модель обучалась восстанавливать скрытые части лиц, выучивая геометрию без учителя.

    • Pseudo-Labeling: Добавление ~8000 "алмазных" примеров из теста с уверенностью >99.5%.

    • Результат: Это позволило пробить потолок 0.91-0.93 и достичь 0.99 на лидерборде.

## Заключение
Исследование подтвердило, что универсальные архитектуры (MobileNet) непригодны для надежной детекции дипфейков. Инвестиции в вычислительные ресурсы (переход на EfficientNet/TitanNet, увеличение разрешения до 384px) и специализированные методы (Attention, SSL) дают экспоненциальный прирост качества, превращая систему из "генератора случайных чисел" в надежный инструмент безопасности.
