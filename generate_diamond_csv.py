...
print(f"GENERATING DIAMOND CSV | Device: {CONFIG['device']}")
...
class TitanNet(nn.Module):
    def __init__(self, in_chans=9, dims=[96, 192, 384, 768], depths=[3, 3, 9, 3]):
        super().__init__()
...
if __name__ == '__main__':
    ...
    print(" Running Inference to find DIAMONDS...")
    final_probs = []
    ids_list = []
    ...
    print(f"   Confident FAKES (> {CONFIG['conf_threshold']}): {cnt_fake}")
    print(f"   Confident REALS (< {1 - CONFIG['conf_threshold']:.3f}): {cnt_real}")
    print(f"   Total Diamonds: {len(pseudo_data)}")
    ...
    df_final = pd.concat([df_orig[['Id', 'target_feature', 'is_pseudo']], df_pseudo])

    df_final.to_csv('train_diamond.csv', index=False)
    print(f" Saved 'train_diamond.csv' with {len(df_final)} samples.")